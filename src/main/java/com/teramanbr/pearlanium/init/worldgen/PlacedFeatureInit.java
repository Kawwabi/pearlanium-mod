...
public class PlacedFeatureInit {
    public static final RegistryKey<PlacedFeature> PEARLED_ORE_KEY = registerKey("end_pearled_ore");

    public static void bootstrap(Registerable<PlacedFeature> context) {
        RegistryEntryLookup<ConfiguredFeature<?, ?>> registryLookup = context.getRegistryLookup(RegistryKeys.CONFIGURED_FEATURE);

        register(context, PEARLED_ORE_KEY, registryLookup.getOrThrow(ConfiguredFeatureInit.PEARLED_ORE_KEY),
                Modifiers.modifiersCount(2,
                        HeightRangePlacementModifier.uniform(YOffset.fixed(0), YOffset.fixed(128))));
    }

    private static RegistryKey<PlacedFeature> registerKey(String name) {
        return RegistryKey.of(RegistryKeys.PLACED_FEATURE, PearlaniumMod.id(name));
    }
